...
> "JPYC is the digitization of the yen. CU is the monetization of computation.
...
> the right tool in the right place — that is the design judgment Forge makes.
...
- Why Forge CU is fundamentally different from all of them
- Five reasons why CU is not on a blockchain
- The Layer 0 / Layer 1 / Layer 2 three-tier hybrid design
- Concrete integration patterns between JPYC/USDC and CU (bridge contracts)

---

## Why This Chapter Exists

Two questions arise almost immediately when Forge's design is explained to someone
familiar with DeFi:
...
assumption. Forge's choice to run CU off-chain looks strange in that context.
...
blockchain can meet. This chapter explains why, and what Forge does instead.
...
money." Forge is programmable — its rules are in protocol code — but its core
...
### Comparison (including CU)

| Dimension | JPYC | USDC | DAI | CBDC | **Forge CU** |
|---|---|---|---|---|---|
...
CU is anomalous on nearly every dimension. This is not accidental — Forge is solving
a fundamentally different problem.

---

## JPYC and CU: Fundamentally Different
...
CU:   physical computation → new currency issued
      → creation of a new economy (computation → CU → computation)
```

JPYC flows top-down: take something that already has value and make it more usable.
CU flows bottom-up: productive physical acts create new monetary units.
...
CU has no fixed peg, but its issuance is hard-linked to physical computation. The only
way to get more CU is to do more useful computation or borrow from the lending pool
...
CU and JPYC are not rivals. They operate in different layers:

- JPYC bridges the human economy into Web3
- CU bridges AI-agent computation into a monetary economy

They *need* each other. When a company wants to fund an AI agent, it sends JPYC to a
bridge contract that credits the agent in CU. When unused CU is returned to the company,
...
## Five Reasons CU is Off-Chain

### 1. Gas Costs Make Small Transactions Uneconomical

A typical Forge inference request costs 0.0001 to 0.01 USD. Ethereum L1 gas for a
...
If CU were an ERC-20 token, someone would create a Uniswap pool within hours. Speculators
would buy expecting appreciation. Price would decouple from compute supply. Providers
would prefer selling CU on the open market to providing inference. The Bittensor (TAO),
...
Forge's design principle is to make speculation structurally impossible, not merely
...
trigger a CU transfer. The required settlement latency is milliseconds.

| System | Settlement latency |
|---|---|
| Forge (off-chain) | ~1 ms |
| Solana | ~400 ms |
| Polygon | ~2 s |
...
TPS. Forge's design requires the gap to be closed by 2–3 orders of magnitude. Off-chain
...
From `forge/CLAUDE.md`:

> **No blockchain in the core.** CU accounting uses local ledgers + gossip + dual
...
│  · JPYC / USDC ↔ CU bridge contracts                 │
│  · Insurance, futures, options (forge-bank roadmap)   │
└──────────────────────────────────────────────────────┘
```

**Layer 0** is where the economy lives. All CU transactions run here. No gas, no
...
This is OpenTimestamps-style anchoring applied to CU trade history.
...
**Step 1 — Deposit:** The company sends 100,000 JPYC to the Forge bridge contract
on Ethereum L2.

**Step 2 — CU issuance:** The bridge contract credits the agent wallets with the
equivalent CU amount.

**Step 3 — Normal operation:** Agents trade inference on Layer 0. No gas, no
confirmation wait.

**Step 4 — Redemption:** Month-end unused CU flows back through the bridge contract
as JPYC.
...
1. JPYC's programmability (conditional payment, multisig, escrow) is available at the
   funding layer
2. CU's off-chain efficiency (zero gas, millisecond settlement) is preserved for all
...
in the DeFi ecosystem (Ronin, Wormhole, Nomad — billions USD lost). Any Forge bridge
...
**Regulatory classification.** CU does not cleanly fit existing regulatory categories
in any jurisdiction. When CU bridges to JPYC or USDC, the fiat-side transaction falls
under existing regulations; the CU side remains in a legal grey area. Clarification
...
1. **CU is not digitized yen or digitized USD.** It is monetized computation.
   JPYC and CU serve different layers and are complementary, not competitive.

2. **CU is off-chain for five structural reasons:** gas economics, speculation isolation,
...
3. **The three-layer hybrid design** allocates work appropriately: Layer 0 for all
   routine CU transactions (off-chain, zero gas); Layer 1 for immutable audit anchoring
